...
PlotMarker <- function(fsom, marker=NULL, view=TRUE,main=NULL, 
    ...
    if(is.null(marker)){
        ...
        igraph::plot.igraph(f$MST$graph, layout=layout, 
            vertex.size=fsom$MST$size, 
...
PlotVariable <- function(fsom, variable, view="MST",main=NULL, 
    ...
    graphics::plot.new()
    if(symmetric){
        legend.col(colorPalette(100),c(-max(abs(variable)),max(abs(variable))))
    } else {
    ...
    f <- fsom
    if(symmetric){
#         print(length(colorPalette(100)[as.numeric(cut(
#             c(-max(abs(variable)),max(abs(variable)),variable), 
#               breaks = 100))[-c(1,2)]]))
        igraph::V(f$MST$graph)$color <- colorPalette(100)[as.numeric(cut(
            c(-max(abs(variable)),max(abs(variable)),variable), 
            breaks = 100))[-c(1,2)]] 
    ...
    igraph::plot.igraph(f$MST$graph, layout=layout, vertex.size=fsom$MST$size, 
        vertex.label=NA, main=main,edge.lty=lty)
...
plotStarLegend <- function(labels, colors=grDevices::rainbow(length(labels)),
    ...
    y <- shift_function(y_tmp,max((cummax(y_tmp)<0)*seq_along(y_tmp)))
        ...
        if(!is.null(backgroundValues)){
            ...
            if(is.factor(backgroundValues)){
                legend("center", legend=levels(backgroundValues),
                        ...
                        legend.col(backgroundColor,c(backgroundLim,
                                                    backgroundValues))
            }
        } else {
            layout(matrix(c(1,2), 1, 2, byrow = TRUE), 
...
PlotClusters2D <- function(fsom, marker1, marker2, nodes, main="",
    ...
    graphics::points(fsom$data[fsom$map$mapping[,1] %in% nodes,
            ...
            pch=".", 
            col=col)
    graphics::points(fsom$map$meanValues[nodes,marker1],
